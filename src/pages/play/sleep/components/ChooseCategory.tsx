    ...
    <FadeWrapper className="grid grid-cols-1 gap-4">
      ...
      <AnimatePresence>
        {categories.map((category, index) => (
          <FadeWrapper
            key={`category-sleep-${index}-${category.key}`}
            delay={(index + 1) * 0.3}
          >
            <Card
              key={`category-sleep-${index}-${category.key}-card`}
              role="button"
              className="cursor-pointer hover:border-primary border border-transparent"
              onClick={() =>
                handleChooseCategory(category.key as CategorySleep)
              }
            >
              <CardHeader>
                {category.icon}
